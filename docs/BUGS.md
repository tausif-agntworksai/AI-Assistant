...
| The wake word needed saying two to four times | The threshold was openWakeWord's default of 0.5. Streaming 20 synthesised "hey jarvis" clips through it in four gain/noise conditions, 18 of 80 utterances peaked *below* 0.5 — and every one of those came from an accented voice, while the en-US and en-GB clips never fell under 0.979. It was the pretrained model's bias meeting a threshold picked for it. Now 0.30, which misses 7 of 80; the only non-wake-word utterance that scores that high is the bare name "jarvis". | `test_wake_tune.py::test_the_shipped_threshold_matches_what_was_measured` pins the number, and [`HEARING.md`](HEARING.md) holds the table |
| Saying the wake word more clearly made it *less* likely to be heard | The model was trained on the phrase as one connected unit, so a pause between "hey" and "jarvis" costs up to 0.86 of score — measured at 0.136 against 0.997 for the same voice. The instinct after being missed is to slow down and separate the words, which is the worst thing to do. `--tune-wake-word` now says so before it starts measuring. | The phrasing table in [`HEARING.md`](HEARING.md) |
| Garbled-but-obvious commands were thrown away | The fuzzy matcher scored the recoverable ones at 70-77 against a threshold of 78, so a whole class of "it did not understand me" was a threshold rather than a model. Lowering it to 70 needed two guards first, below. | `test_mangled_transcripts.py`, built from the transcripts a real recogniser produced |
| A fuzzy match could invert a direction | `token_sort_ratio` scored a garbled "brightness kam karo" at 72 against **`brightness_up`** — the opposite of what was said. Turning the screen up when asked to turn it down is worse than not understanding. A match on a skill that has an opposite now requires a word that actually distinguishes the two, derived from the skills' own `examples` rather than a hand-kept synonym list. | `test_a_clear_direction_is_never_inverted`, `test_an_ambiguous_direction_is_refused_rather_than_guessed` |
| A fuzzy match could invent a magnitude | The guard against argument-less fuzzy matches only checked for *required* parameters, and `set_brightness(level: int = 60)` has a default — so a 76-scoring "brightness calm" silently set the screen to 60%. Intent can be inferred; a number has to be in the utterance. | `test_a_fuzzy_match_never_supplies_a_magnitude` |
| Tuning `brain.rules_threshold` changed one caller and nothing else | `route()` carried its own default of `78.0` in the signature, duplicating the config value. The two had already drifted: config said 70, the signature said 78, so the setting only affected the orchestrator — every other caller, and every test, silently used the stale number. It now resolves from config. | `test_mangled_transcripts.py` calls `route()` with no threshold, so the two cannot diverge again |
| "turn the brightness down" was not understood | English splits a phrasal verb around its object as readily as it keeps it together, and only "turn down the brightness" had a rule. Volume had the same hole and was being papered over by an example match. | `test_a_clear_direction_is_never_inverted`, which includes both word orders |
| Losing the internet said "That didn't work." | The four network-dependent skills failed through the generic exception handler, whose message is indistinguishable from a bug — so the user repeats a command that cannot succeed. Skills now declare `needs_network` and are refused up front with the feature named and a note that the rest still works. The check runs only for skills that asked for it, so a local command never waits on a socket, and an inconclusive check counts as online. | `test_net.py` — including `test_a_local_skill_never_triggers_a_connectivity_check` and `test_being_unsure_counts_as_online` |
| An expired API key sent the user to check their wi-fi | `looks_like_connectivity` was the fix and also the trap: a 401, a 404 or a rate limit is a working connection delivering bad news. The predicate is deliberately narrow, and `requests`/`httpx` wrappers are matched by qualified name because `isinstance` against the socket classes misses them. | `test_an_ordinary_error_is_not_blamed_on_the_network`, `test_a_wrapped_requests_error_is_still_recognised` |
| "whatsapp chalu karo" sent a message to a contact called "chalu karo" | The new bare `whatsapp <who>` messaging rule read the app name as the verb. Guarded with a negative lookahead on launch/close verbs — an app name followed by one of those is app control, never messaging. | `test_what_must_not_become_a_message`, which lists every phrasing that regressed during development |
| The guard was silently disabled by an invisible byte | `\b` written into a **non-raw** Python string is a backspace (0x08), not a word boundary. One got into `_APP_VERBS` from a patch script, so the pattern required an unmatchable control character and the guard never fired — while `grep` showed nothing wrong, because the byte is invisible. | The guard's own tests fail if it stops matching; a repo-wide sweep removed the stray control characters |
| "tell amit I'm running late" arrived as "im running late" | Rule builders only ever saw normalised text, which is correct for a skill name and wrong for a sentence quoted to another person. Builders may now take a third parameter and get the raw utterance; the body is recovered by anchoring on its first and last word. | `test_the_message_keeps_the_words_the_user_actually_said` |
| "batao kitna baja hai" answered with the battery level | Every time rule is anchored to the start of the utterance, so a leading "batao" pushed it to the fuzzy matcher, which scored 69.8 against "battery kitni bachi hai". Hindi request markers are now leading filler — leading-only, because "mummy ko batao ki…" needs its "batao" as the verb the message rule matches on. | `test_what_must_not_become_a_message[batao kitna baja hai-get_time]` |
| Permissions could be granted once and never revoked | The consent screen ran at setup and had no way back: changing your mind about the microphone meant deleting a file nobody tells you about. There is now a Permissions section in Settings that applies each toggle immediately and re-reads the engine's state rather than trusting the local copy. | Covered by the existing consent API tests; the section reuses `consent.list()` / `consent.save()` |
| The wake-word cue sounded clipped and mechanical | It was rendered at +40% speech rate to keep it short, since the frames it occupies are dropped and a shorter cue discards less of the next command. It measured well and sounded wrong. Now −5% with a 25 ms attack and a 90 ms release, levelled to a common peak — the abrupt onset was as much of it as the rate, because trimming the synthesiser's padding left the clip starting mid-waveform. | `test_the_cue_fades_in_rather_than_switching_on`, `test_every_variant_is_levelled_to_the_same_peak` |
| One cue variant was 43% louder than the next | The variants rotate, so an inconsistent level is heard as a glitch rather than as variety. Every cue is now levelled to `CUE_PEAK`. | `test_every_variant_is_levelled_to_the_same_peak` |
| A cached cue survived the fix that replaced it | The cache filename keyed on the speech rate only, so a machine that had run the old version kept playing the old clip and the fix appeared not to work. The key now covers the shaping too. | `test_the_cache_key_changes_when_the_shaping_does` |
| The first photo of a session was a dark rectangle | Nearly every webcam returns two or three badly exposed frames while auto-exposure and auto-white-balance converge, and the capture took the first one. Eight are now grabbed and discarded. | `test_a_photo_is_taken_from_a_settled_frame` |
| The webcam light stayed on after taking one photo | The device was released on the success path only. It is now released in a `finally`, on every path including each failure. | `test_the_device_is_released_when_no_frame_ever_arrives`, `test_the_device_is_released_when_reading_raises` |
...
- **`\b` in a non-raw Python string is a backspace, not a word boundary.** It
  writes an invisible 0x08 into whatever it touches, which `grep` will not show
  you and which silently breaks any regex containing it. The same applies to
  `\n`, `\r` and `\f`. When generating Python source from a script, use raw
  strings or build the escape with `chr(92) + "b"`.
- **A lazily-imported module is missing from the frozen build.** PyInstaller
  only follows imports it can see, so anything imported inside a function has to
  be named in `hiddenimports` — and for a diagnostic, it goes missing exactly
  when someone is already debugging something else.
